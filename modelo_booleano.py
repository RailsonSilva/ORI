...
consulta_read_filter = consulta_read.split() #
...
print("aqui")
print(consulta_read_filter) # imprime - ['cas', '&', 'am', '|', 'cas', '&', '!mor']
print("aqui")

j = 0
while j < len(consulta_read_filter):

    if(consulta_read_filter[j] in ("&")):
        if(consulta_read_filter[j-1] and consulta_read_filter[j+1] in palavras):
            print(consulta_read_filter[j-1])
            print(consulta_read_filter[j+1])
